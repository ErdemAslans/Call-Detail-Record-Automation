...
import TableEntriesInfo from "./table-content/table-footer/TableEntriesInfo.vue";
import {
  computed,
  defineComponent,
  onMounted,
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const startEntry = computed(() => {
      return (page.value - 1) * itemsCountInTable.value + 1;
    });

    const endEntry = computed(() => {
      return Math.min(page.value * itemsCountInTable.value, props.count);
    });
